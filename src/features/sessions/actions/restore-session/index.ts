...
import { restoreSessionSchema } from "./schema";
import type { RestoreSessionInput, RestoreSessionResult } from "./types";

export async function restoreSessionAction(
  input: RestoreSessionInput,
): Promise<RestoreSessionResult> {
  const parsed = restoreSessionSchema.safeParse(input);
  if (!parsed.success) {
